...
import Node.Crypto.Types (Ciphertext(..))
...
main = do
  ...
  hexCipher <- Cipher.hex Types.AES256 key (Just iv) sample_plaintext
  fromHexDecipher <- Decipher.fromHex Types.AES256 key (Just iv) Nothing hexCipher.ciphertext
  base64Hash <- Hash.base64 Hash.SHA512 "sample_password"
  base64Hmac <- Hmac.base64 Hash.SHA512 secret "sample_password"
  base64Cipher <- Cipher.base64 Types.AES256 key (Just iv) sample_plaintext
  fromBase64Decipher <- Decipher.fromBase64 Types.AES256 key (Just iv) Nothing base64Cipher.ciphertext
  base64CipherIv <- Cipher.base64 (Types.WithAuth Types.AES256GCM) key (Just iv) sample_plaintext
  fromBase64DecipherIv <- Decipher.fromBase64 (Types.WithAuth Types.AES256GCM) key (Just iv) (base64CipherIv.authTag) (base64CipherIv.ciphertext)
  ...
  assert $ hexCipher.ciphertext == Ciphertext "cf4e5bb9d709c6e62267870b52ca691d67b14ba4aeb3b0127c24aadaf4d73eb2"
  ...
  assert $ base64Cipher.ciphertext == Ciphertext "z05budcJxuYiZ4cLUsppHWexS6Sus7ASfCSq2vTXPrI="
  assert $ fromBase64Decipher == sample_plaintext
