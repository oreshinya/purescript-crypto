...
import Node.Crypto.Hmac (createHmac, digest, update, Secret(..))
...
timingSafeEqualString x1 x2 = do
  ...
  b1 <- createHmac SHA256 (Secret secret) >>= flip update a1 >>= digest
  b2 <- createHmac SHA256 (Secret secret) >>= flip update a2 >>= digest
